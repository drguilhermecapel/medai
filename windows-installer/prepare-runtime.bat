...
if not exist "%RUNTIME_DIR%\postgresql" (
    ...
    if %ERRORLEVEL% NEQ 0 (
        ...
        if %POWERSHELL_AVAILABLE% EQU 1 (
            ...
            certutil -urlcache -split -f "https://ftp.postgresql.org/pub/binary/v15.7/win32/postgresql-15.7-1-windows-x64-binaries.zip" "%TEMP_DIR%\postgresql.zip" >nul 2>&1
            if %ERRORLEVEL% NEQ 0 (
                bitsadmin /transfer "PostgreSQLAltDownload" /download /priority normal "https://ftp.postgresql.org/pub/binary/v15.7/win32/postgresql-15.7-1-windows-x64-binaries.zip" "%TEMP_DIR%\postgresql.zip" >nul 2>&1
            )
        )
...
if not exist "%~dp0redist\VC_redist.x64.exe" (
    ...
    if %POWERSHELL_AVAILABLE% EQU 1 (
        ...
        certutil -urlcache -split -f "https://aka.ms/vs/17/release/vc_redist.x64.exe" "%~dp0redist\VC_redist.x64.exe" >nul 2>&1
    )
