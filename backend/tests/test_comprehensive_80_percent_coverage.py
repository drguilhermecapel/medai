...
async def test_ecg_endpoints_comprehensive_coverage(client, auth_headers):
    ...
    with patch('app.api.v1.endpoints.ecg_analysis.UserService.get_current_user') as mock_user, \
        ...
        client.get("/api/v1/ecg/test123", headers=auth_headers)
        ...
        client.get("/api/v1/ecg/test123/measurements", headers=auth_headers)
        client.get("/api/v1/ecg/test123/annotations", headers=auth_headers)

        mock_service.return_value.delete_analysis.return_value = True
        client.delete("/api/v1/ecg/test123", headers=auth_headers)
        ...
        client.get("/api/v1/ecg/critical/pending", headers=auth_headers)
...
async def test_ecg_service_processing_methods():
    ...
    with patch('neurokit2.ecg_process') as mock_process:
        ...
        await service._extract_measurements(ecg_data, sample_rate)
    ...
    with patch('neurokit2.ecg_process') as mock_process:
        ...
        await service._generate_annotations(ecg_data, ai_results, sample_rate)
    ...
    await service._assess_clinical_urgency(ai_results)
...
async def test_repositories_comprehensive_coverage():
    ...
    await ecg_repo.update_analysis_status(1, "completed")

    mock_result = Mock()
    mock_result.scalars.return_value.all.return_value = []
    ecg_repo.db.execute = AsyncMock(return_value=mock_result)

    await ecg_repo.get_critical_analyses(limit=20)
...
async def test_api_endpoints_comprehensive_coverage(client, auth_headers):
    ...
    with patch('app.api.v1.endpoints.users.UserService.get_current_user') as mock_user:
        ...
        client.put("/api/v1/users/me", headers=auth_headers, json={"full_name": "Test User"})
        client.post("/api/v1/users/me/change-password", headers=auth_headers,
                             json={"current_password": "old", "new_password": "new"})

        mock_user.return_value = Mock(id=1, is_superuser=True)
        client.get("/api/v1/users/", headers=auth_headers)
        client.get("/api/v1/users/123", headers=auth_headers)
    ...
    with patch('app.api.v1.endpoints.validations.UserService.get_current_user') as mock_user, \
        ...
        client.post("/api/v1/validations/",
            headers=auth_headers,
            json={"analysis_id": 1, "validation_type": "clinical"}
        )

        client.get("/api/v1/validations/my-validations", headers=auth_headers)
        client.post("/api/v1/validations/1/submit", headers=auth_headers,
                             json={"comments": "Test"})
        client.get("/api/v1/validations/1", headers=auth_headers)
        client.get("/api/v1/validations/pending/critical", headers=auth_headers)
    ...
    with patch('app.api.v1.endpoints.notifications.UserService.get_current_user') as mock_user, \
        ...
        client.get("/api/v1/notifications/", headers=auth_headers)
        client.post("/api/v1/notifications/1/read", headers=auth_headers)
        client.post("/api/v1/notifications/mark-all-read", headers=auth_headers)
        client.get("/api/v1/notifications/unread-count", headers=auth_headers)
    ...
    with patch('app.api.v1.endpoints.patients.UserService.get_current_user') as mock_user, \
        ...
        client.get("/api/v1/patients/", headers=auth_headers)
        client.get("/api/v1/patients/1", headers=auth_headers)
        client.put("/api/v1/patients/1", headers=auth_headers,
                            json={"full_name": "Updated Patient"})
        client.delete("/api/v1/patients/1", headers=auth_headers)
