...
class TestNotificationEndpointsCoverage:
    ...
    async def test_notification_preferences(self, client, authorized_headers):
        ...
        assert response.status_code == 200
        assert response.json()["email_enabled"]
...
class TestSecurityFunctions:
    ...
    def test_password_complexity_validation(self):
        ...
        assert validate_password("C0mpl3x!Pass")

        # Invalid passwords
        assert not validate_password("weak")
        assert not validate_password("NoNumbers!")
        assert not validate_password("nouppercas3!")
    ...
    def test_token_scope_validation(self):
        ...
        # Verify scopes
        assert verify_token_scope(token, "read:ecg")
